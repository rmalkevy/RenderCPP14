...
// за допомогою якого я буду вираховувати тільки один раз нормаль та hitPoint
// Тобто я пройдусь по всіх об'єктах і визначу, який найближчий, а потім уже буду
// шукати нормаль і hitPoint
}
...
//	Vec3d rotPositionCamera = render->camera->rotateVec3d(render->camera->getPosition(), Vec3d(0, 0.3, 0));
    for (int y = -halfHeight; y < halfHeight; y++)
    {
        for (int x = -halfWidth; x < halfWidth; x++)
        {
            // TODO потім дописати поворот pixel
	        Vec3d pixel = render->camera->rotateVec3d(Vec3d(x, y, 0), Vec3d(0, 0, 0));
...
//	        std::cout << "dir X = " << dir.GetX() << " Y = " << dir.GetY() << " Z = " << dir.GetZ() << std::endl;
	        render->camera->setMaxDistance(MaxDistance);
            ...
            {
	            // TODO: тут також потрибно оптимізувати код, бо ця функція tracingLight виглядає тупо
                render->window->PutColorToPixel(tracingLight(render->camera, render->listPrimitives), x, y);
            }
        }
    }
}
...
int main()
{
	...
	// Create light
	Light light(Vec3d(0, 0, -2000), Vec3d(1, 1, 1));
